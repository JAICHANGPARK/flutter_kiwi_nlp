...
model provisioning. Empirically, repeated benchmark trials show a
\textit{warm-path throughput reversal} (steady-state path, initialization
excluded): Flutter warm throughput can exceed the Python-native baseline:
3.12x on desktop, 3.87x on iOS simulator (debug), and 1.52x on Android emulator
(release). Added Linux/Windows CI rows show clear execution-mode sensitivity:
single mode is 0.64x of Python throughput on both OSes, while batch mode is
1.54--1.55x faster than Python on the same runners. The updated benchmark
pipeline also adds boundary-decomposed
measurements (pure processing vs full JSON path), quantifying
serialization/parsing overhead at 13.89--47.93\% depending on runtime profile.
This warm-path reversal does not imply end-to-end session superiority:
cold-start initialization and short-session
effective-throughput remain weaker than the Python baseline on all measured
platforms. Cross-runtime linguistic agreement remains close on
gold corpora
...
applications. This revision also contributes glass-box performance profiling via
boundary-decomposed metrics (pure vs full path) and cross-OS CI benchmark rows
that expose execution-mode sensitivity. It does not claim a new morphological
decoding algorithm or a new Korean language model architecture.
...
own benchmark, \texttt{flutter\_kiwi\_nlp} can outperform Python-native
\texttt{kiwipiepy} on warm-path throughput under the latest measured profile,
but still shows slower cold start and lower short-session effective throughput
because initialization and bridge costs are front-loaded.
...
For reproducible Web TTI reporting in the next revision, we will publish
cold-cache and warm-cache TTI distributions (p50/p95) under explicit network
profiles, together with module/WASM/model artifact sizes and cache-hit/miss
annotations.
...
Repository commit base & \texttt{2ab94d2b3fa6a941c8a339547e4e1797695ab06d} \\
...
Desktop benchmark shape & \texttt{trials=3, warmup\_runs=3, measure\_runs=15, top\_n=1} \\
Desktop analyze impl & Flutter: \texttt{json} (primary), Kiwi: \texttt{analyze} \\
Linux/Windows CI runner shape & \texttt{trials=3, warmup\_runs=3, measure\_runs=15, top\_n=1} \\
Linux/Windows CI analyze impl & Flutter: \texttt{token\_count} (primary), Kiwi: \texttt{analyze} \\
\bottomrule
\end{longtable}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  (release, \texttt{n=3}, \texttt{warmup=3}, \texttt{measure=15}).
  \item Mobile platform A: iOS 26.2 simulator (iPhone 17, debug,
  \texttt{n=3}, \texttt{warmup=3}, \texttt{measure=15}).
  \item Mobile platform B: Android 16 emulator (API 36, release,
  \texttt{n=3}, \texttt{warmup=3}, \texttt{measure=15}).
  \item Desktop CI platform A: Linux GitHub runner
  (\texttt{n=3}, \texttt{warmup=3}, \texttt{measure=15}).
  \item Desktop CI platform B: Windows GitHub runner
  (\texttt{n=3}, \texttt{warmup=3}, \texttt{measure=15}).
  ...
  all reported rows use \texttt{15 * 40 = 600}.
  \item \texttt{top\_n}: 1.
  \item Desktop primary analyze path:
  Flutter \texttt{json}, Kiwi \texttt{analyze}
...
\subsection{Recorded Result Summary: Desktop Baseline (macOS, n=3)}
...
(\texttt{n = 3}, two-sided \texttt{t}-interval, \texttt{df = 2}).
\[
...
Therefore, the Android 4.03x median gap should be read as combined startup
cost, not as a JNI/FFI-only penalty estimate.

\subsection{Additional Mobile Runtime Measurements (iOS n=3, Android n=3)}
...
\midrule
\endhead
Throughput (analyses/s) & 9890.53 ± 3756.49 & 2553.35 ± 61.83 & 3.87x faster \\
Throughput (chars/s) & 335288.96 ± 127345.03 & 86558.61 ± 2096.03 & 3.87x faster \\
Throughput (tokens/s) & 159237.53 ± 60479.50 & 41045.12 ± 993.92 & 3.88x faster \\
Avg latency (ms, lower better) & 0.11 ± 0.05 & 0.39 ± 0.01 & 0.29x faster \\
Avg token latency (us/token, lower better) & 7.11 ± 3.30 & 24.37 ± 0.58 & 0.29x faster \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Throughput (analyses/s) & 3865.71 ± 811.36 & 2543.80 ± 69.61 & 1.52x faster \\
Throughput (chars/s) & 131047.62 ± 27505.16 & 86234.83 ± 2359.65 & 1.52x faster \\
Throughput (tokens/s) & 62237.96 ± 13062.92 & 40891.59 ± 1118.92 & 1.52x faster \\
Avg latency (ms, lower better) & 0.27 ± 0.06 & 0.39 ± 0.01 & 0.68x faster \\
Avg token latency (us/token, lower better) & 16.62 ± 3.96 & 24.47 ± 0.68 & 0.68x faster \\
\bottomrule
\end{longtable}

\subsection{Additional Desktop CI Measurements (Linux/Windows, n=3)}
\begingroup
\footnotesize
\setlength{\tabcolsep}{2pt}
\begin{longtable}{@{}L{0.18\linewidth}L{0.13\linewidth}L{0.13\linewidth}L{0.10\linewidth}L{0.13\linewidth}L{0.13\linewidth}L{0.10\linewidth}@{}}
\caption{Linux/Windows GitHub runner warm-path summary (single vs batch, mean ± stddev)}\label{tab:benchmark-desktop-ci-warm}\\
\toprule
Platform/Mode & Flutter APS & Kiwi APS & Ratio & Flutter Lat (ms) & Kiwi Lat (ms) & Ratio \\
\midrule
\endfirsthead
\toprule
Platform/Mode & Flutter APS & Kiwi APS & Ratio & Flutter Lat (ms) & Kiwi Lat (ms) & Ratio \\
\midrule
\endhead
Linux/single & 2084.75 ± 74.15 & 3244.11 ± 48.99 & 0.64x & 0.48 ± 0.02 & 0.31 ± 0.00 & 1.56x slower \\
Linux/batch & 5129.13 ± 56.53 & 3321.86 ± 6.59 & 1.54x & 0.19 ± 0.00 & 0.30 ± 0.00 & 0.65x faster \\
Windows/single & 1921.24 ± 5.33 & 3018.30 ± 67.70 & 0.64x & 0.52 ± 0.00 & 0.33 ± 0.01 & 1.57x slower \\
Windows/batch & 4622.39 ± 35.36 & 2987.51 ± 23.77 & 1.55x & 0.22 ± 0.00 & 0.33 ± 0.00 & 0.65x faster \\
\bottomrule
\end{longtable}
\endgroup
...
\footnotesize
\setlength{\tabcolsep}{2pt}
\begin{longtable}{@{}L{0.20\linewidth}L{0.17\linewidth}L{0.19\linewidth}L{0.18\linewidth}L{0.18\linewidth}@{}}
\caption{Linux/Windows GitHub runner boundary/cold/session summary}\label{tab:benchmark-desktop-ci-overview}\\
\toprule
Platform/Mode & Boundary Loss & Ovh./analysis (ms) & Init Ratio (median) & N=1000 Eff. Ratio \\
\midrule
\endfirsthead
\toprule
Platform/Mode & Boundary Loss & Ovh./analysis (ms) & Init Ratio (median) & N=1000 Eff. Ratio \\
\midrule
\endhead
Linux/single & 13.89\% & 0.0779 ± 0.0114 & 2.69x slower & 0.42x \\
Linux/batch & 22.22\% & 0.0557 ± 0.0025 & 2.71x slower & 0.49x \\
Windows/single & 15.85\% & 0.0982 ± 0.0094 & 2.67x slower & 0.42x \\
Windows/batch & 23.12\% & 0.0651 ± 0.0054 & 2.54x slower & 0.47x \\
\bottomrule
\end{longtable}
\endgroup
...
\midrule
\endhead
macOS desktop baseline & 1 & 0.60 ± 0.01 & 1.11 ± 0.02 & 0.54x \\
macOS desktop baseline & 10 & 6.01 ± 0.06 & 11.10 ± 0.22 & 0.54x \\
macOS desktop baseline & 100 & 59.65 ± 0.60 & 106.60 ± 1.97 & 0.56x \\
macOS desktop baseline & 1000 & 557.18 ± 5.11 & 765.03 ± 8.08 & 0.73x \\
iOS simulator (debug) & 1 & 0.58 ± 0.00 & 1.16 ± 0.03 & 0.50x \\
iOS simulator (debug) & 10 & 5.75 ± 0.04 & 11.56 ± 0.32 & 0.50x \\
iOS simulator (debug) & 100 & 57.17 ± 0.42 & 111.05 ± 2.87 & 0.51x \\
iOS simulator (debug) & 1000 & 540.24 ± 15.79 & 797.83 ± 9.76 & 0.68x \\
Android emulator (release) & 1 & 0.24 ± 0.01 & 0.96 ± 0.09 & 0.25x \\
Android emulator (release) & 10 & 2.42 ± 0.08 & 9.59 ± 0.87 & 0.25x \\
Android emulator (release) & 100 & 24.03 ± 0.79 & 92.73 ± 8.19 & 0.26x \\
Android emulator (release) & 1000 & 227.05 ± 5.21 & 697.44 ± 46.98 & 0.33x \\
\bottomrule
\end{longtable}
...
\addplot+[
  ...
  (Analyses/s,311.80) +- (0,11.34)
  (Chars/s,311.80) +- (0,11.34)
  (Tokens/s,312.29) +- (0,11.36)
};
\addplot+[
  fill=orange!60,
  ...
  (Analyses/s,100.00) +- (0,1.21)
  (Chars/s,100.00) +- (0,1.21)
  (Tokens/s,100.00) +- (0,1.21)
};
...
\addplot+[
  ...
  (Init(ms),186.62) +- (0,0.63)
  (AvgLatency(ms),31.71) +- (0,0.00)
  (TokenLatency(us/token),32.04) +- (0,1.14)
};
\addplot+[
  fill=orange!60,
  draw=orange!80!black,
  ...
  (Init(ms),100.00) +- (0,2.17)
  (AvgLatency(ms),100.00) +- (0,0.00)
  (TokenLatency(us/token),100.00) +- (0,1.22)
};
\legend{flutter\_kiwi\_nlp,kiwipiepy}
\end{axis}
\end{tikzpicture}
...
configurations (roughly 0.1--0.6 ms). This matches the practical impression
that repeated in-app analysis is responsive after initialization. At the same
time, repeated measurements still show slower init on the Flutter path relative
to the Python reference runtime, with magnitude varying by environment.
In the latest run profile, warm-path throughput itself reverses direction and
is higher on Flutter across the macOS+iOS+Android baseline rows:
desktop \(3.12\times\), iOS simulator \(3.87\times\), and Android emulator
\(1.52\times\) versus \texttt{kiwipiepy}. Compared with the prior table values
in this report, Flutter warm analyses/s increased sharply
(desktop: \(2546.24 \rightarrow 7606.26\), +198.73\%;
iOS: \(2422.31 \rightarrow 9890.53\), +308.32\%;
Android: \(2214.79 \rightarrow 3865.71\), +74.54\%).
However, session-length effective throughput remains lower for short sessions
because cold-start cost is still front-loaded on the Flutter path.

New desktop CI rows on Linux/Windows add an additional signal: execution mode
is a first-order factor. In both OSes, single mode shows lower warm throughput
than Python (\(\approx 0.64\times\)), while batch mode flips the direction and
exceeds Python (\(\approx 1.54\)--\(1.55\times\)). This pattern is consistent
with reduced per-call boundary overhead under batch dispatch, while init-heavy
short sessions remain disadvantaged (Linux/Windows \(N=1000\) effective ratio
still around \(0.42\)--\(0.49\times\)).

New boundary-decomposed instrumentation
(Tables~\ref{tab:benchmark-boundary} and
\ref{tab:benchmark-desktop-ci-overview}) adds direct evidence for where
overhead accumulates. Flutter boundary loss (full JSON path vs pure processing
path) spans 13.89--47.93\% across the measured rows, with per-analysis overhead
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  differ in the current macOS baseline row (Flutter 9660 vs Python 9645),
  indicating backend-wrapper semantic differences even under aligned knobs.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  (all reported rows use \texttt{n=3}).
  \item No explicit CPU pinning or thermal-state control.
  \item Linux/Windows desktop CI rows were collected on shared GitHub-hosted
  runners, so VM scheduling variability can affect jitter and tail latency.
  ...
  \item Physical-device benchmark commands are provided in Appendix C, but this
  revision does not yet include executed iOS/Android physical-device rows.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Warm-path throughput can exceed Python-native execution in this
  revision, but short-session effective throughput still trails because
  initialization dominates small-\(N\) workloads.
  \item Bridge-level JSON serialization overhead in the native C layer remains
  a major cost center (dynamic allocation and string copy on hot paths).
...
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Could zero-copy or binary bridge further improve throughput and close effective-session gaps?}
  It is a promising direction and is explicitly planned. Current boundary
  measurements indicate JSON-path overhead around 13.89--47.93\% on warm paths; this
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  To make this roadmap auditable, the next revision will use explicit target
  thresholds against the current baseline:
  \begin{itemize}[leftmargin=1.5em]
    \item \textbf{Primary target:} at least 40\% relative reduction in
    boundary loss (Table~\ref{tab:benchmark-boundary}) from the current
    36.85\%/47.93\%/19.95\% (desktop/iOS/Android) to
    \(\le\)22.11\%/\(\le\)28.76\%/\(\le\)11.97\%.
    \item \textbf{Primary target:} at least 35\% relative reduction in
    boundary overhead per analysis from
    0.0485/0.0891/0.0810 ms to
    \(\le\)0.0315/\(\le\)0.0579/\(\le\)0.0527 ms.
    \item \textbf{Stretch target:} session-length effective throughput ratio
    at \(N=1000\) (Table~\ref{tab:benchmark-session-effective}) to reach
    \(\ge\)0.85x/\(\ge\)0.80x/\(\ge\)0.45x
    (desktop/iOS/Android) from current 0.73x/0.68x/0.33x.
    \item \textbf{Quality guardrails:} token/POS agreement regression
    \(\le\)0.2 percentage points, no increase in crash/timeout incidence, and
    peak RSS increase \(\le\)10\%.
    \item \textbf{Statistical acceptance:} \(n \ge 10\) repeated trials per
    target and 95\% CI reporting; an optimization is accepted only when both
    the mean improvement and the CI lower bound exceed zero.
  \end{itemize}
  ...
  controls and explicit startup-path annotations (asset-cache hit/miss). The
  minimum publication matrix is one recent iPhone (A-series) and one recent
  Android flagship (Snapdragon class), each with \(n \ge 10\) repeated trials in
  release mode and reported p50/p95 plus 95\% CI. Include sustained Web
  throughput and first-use TTI rows under controlled network/cache conditions
  (cold-cache and warm-cache, plus explicit throttled/unthrottled profiles).
...
performance trade-offs are transparent: warm-path Flutter throughput can exceed
the Python baseline in measured settings (desktop, iOS simulator, Android
emulator, and Linux/Windows batch CI rows), while cold-start and short-session
effective throughput remain lower. Linux/Windows single-mode rows are still
below Python throughput, reinforcing execution-mode dependence as a key systems
factor. Boundary-decomposed profiling still identifies a persistent JSON-path
penalty (roughly 13.89--47.93\% in this report) as a primary optimization
target.
...
# Linux desktop CI row (single mode)
uv run python tool/benchmark/run_compare.py \
  --device linux --mode release --trials 3 \
  --warmup-runs 3 --measure-runs 15 \
  --flutter-analyze-impl token_count \
  --flutter-execution-mode single \
  --output-dir benchmark/results/gha_linux_single_t3_w3_m15_s10

# Linux desktop CI row (batch mode)
uv run python tool/benchmark/run_compare.py \
  --device linux --mode release --trials 3 \
  --warmup-runs 3 --measure-runs 15 \
  --flutter-analyze-impl token_count \
  --flutter-execution-mode batch \
  --output-dir benchmark/results/gha_linux_batch_t3_w3_m15_s10

# Windows desktop CI row (single mode)
uv run python tool/benchmark/run_compare.py \
  --device windows --mode release --trials 3 \
  --warmup-runs 3 --measure-runs 15 \
  --flutter-analyze-impl token_count \
  --flutter-execution-mode single \
  --output-dir benchmark/results/gha_windows_single_t3_w3_m15_s10

# Windows desktop CI row (batch mode)
uv run python tool/benchmark/run_compare.py \
  --device windows --mode release --trials 3 \
  --warmup-runs 3 --measure-runs 15 \
  --flutter-analyze-impl token_count \
  --flutter-execution-mode batch \
  --output-dir benchmark/results/gha_windows_batch_t3_w3_m15_s10
...
\begin{itemize}[leftmargin=1.5em]
  \item \path{benchmark/results/macos_release_t3_json_batch_v2/comparison.md}
  \item \path{benchmark/results/gha_linux_single_t3_w3_m15_s10/comparison.md}
  \item \path{benchmark/results/gha_linux_batch_t3_w3_m15_s10/comparison.md}
  \item \path{benchmark/results/gha_windows_single_t3_w3_m15_s10/comparison.md}
  \item \path{benchmark/results/gha_windows_batch_t3_w3_m15_s10/comparison.md}
  \item \path{benchmark/results/ios_debug_t3_token_count_s10_isolate_worker_v1/comparison.md}
  \item \path{benchmark/results/android_release_t3_token_count_s10_isolate_worker_v1/comparison.md}
  \item \path{benchmark/results/macos_release_t3_json_batch_v2/flutter_kiwi_benchmark_trials.json}
  \item \path{benchmark/results/macos_release_t3_json_batch_v2/kiwipiepy_benchmark_trials.json}
