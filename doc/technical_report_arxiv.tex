...
\hypersetup{
  ...
  pdfauthor={Jai-Chang Park (Google Developer Expert, Dart \& Flutter)}
}
...
approximately 0.71x on same-host desktop baseline, and as low as 0.60x in
mobile engineering-reference rows that compare emulator/simulator Flutter runs
against host macOS Python. Cross-runtime linguistic agreement remains close on
gold corpora
...
\mathcal{B}_t=\mathrm{TopK}\!\left(\mathcal{P}_t(G),K,S\right),
\]
where \(\mathcal{P}_t(G)\) is the partial-path set at depth \(t\), and
\(\mathcal{B}_t\) is the retained beam-like frontier.
...
\begin{tabularx}{\linewidth}{@{}L{0.12\linewidth}L{0.20\linewidth}L{0.14\linewidth}L{0.14\linewidth}L{0.08\linewidth}L{0.08\linewidth}L{0.06\linewidth}@{}}
\toprule
...
\begin{itemize}[leftmargin=1.5em]
  ...
  (\texttt{FlutterKiwiFfiBindings}).
  \item Better maintenance cost profile for long-lived plugin evolution across
...
\midrule
\endhead
Report snapshot date & February 17, 2026 \\
Repository commit & \texttt{f38f79efabcdfca7ab5c54770e79266066c9a6b1} \\
...
From the benchmark trial sets included in this report (February 17, 2026):
\begin{itemize}[leftmargin=1.5em]
  ...
  \item iOS/Android rows were rerun with expanded trial/loop budget
  (\texttt{n=10}, \texttt{warmup=5}, \texttt{measure=30}) to reduce
  single-run noise.
\end{itemize}
...
\midrule
\endhead
Throughput (analyses/s) & 2409.52 ± 135.38 & 3370.35 ± 69.78 & 0.71x \\
Throughput (chars/s) & 81682.66 ± 4589.43 & 114254.92 ± 2365.38 & 0.71x \\
Throughput (tokens/s) & 38793.24 ± 2179.64 & 54178.40 ± 1121.64 & 0.72x \\
Avg latency (ms, lower better) & 0.42 ± 0.02 & 0.30 ± 0.01 & 1.40x slower \\
Avg token latency (us/token, lower better) & 25.84 ± 1.48 & 18.46 ± 0.38 & 1.40x slower \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Throughput (analyses/s) & [2241.42, 2577.62] & [3283.71, 3456.99] \\
Throughput (chars/s) & [75984.13, 87381.19] & [111317.91, 117191.93] \\
Throughput (tokens/s) & [36086.86, 41499.62] & [52785.70, 55571.10] \\
Avg latency (ms) & [0.395, 0.445] & [0.288, 0.312] \\
Avg token latency (us/token) & [24.00, 27.68] & [17.99, 18.93] \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
macOS desktop baseline & median 1367.82, p95 1419.09 & median 670.98, p95 801.21 & 2.04x slower \\
iOS simulator (debug) & median 1439.55, p95 1660.40 & median 749.96, p95 931.24 & 1.92x slower \\
Android emulator (release) & median 4093.05, p95 5573.90 & median 1098.48, p95 1307.90 & 3.73x slower \\
\bottomrule
\end{longtable}

\subsection{Additional Mobile Runtime Measurements (iOS n=10, Android n=10)}
...
\midrule
\endhead
Throughput (analyses/s) & 2065.58 ± 243.45 & 3423.38 ± 181.54 & 0.60x \\
Throughput (chars/s) & 70023.12 ± 8253.07 & 116052.70 ± 6154.21 & 0.60x \\
Throughput (tokens/s) & 33255.82 ± 3919.60 & 55030.89 ± 2918.26 & 0.60x \\
Avg latency (ms, lower better) & 0.49 ± 0.06 & 0.29 ± 0.02 & 1.67x slower \\
Avg token latency (us/token, lower better) & 30.45 ± 3.65 & 18.22 ± 1.01 & 1.67x slower \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Throughput (analyses/s) & 1981.83 ± 250.89 & 3222.94 ± 415.19 & 0.61x \\
Throughput (chars/s) & 67183.98 ± 8505.33 & 109257.81 ± 14074.97 & 0.61x \\
Throughput (tokens/s) & 31907.43 ± 4039.41 & 51808.83 ± 6674.19 & 0.62x \\
Avg latency (ms, lower better) & 0.51 ± 0.08 & 0.32 ± 0.05 & 1.63x slower \\
Avg token latency (us/token, lower better) & 31.88 ± 4.84 & 19.64 ± 2.97 & 1.62x slower \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
macOS desktop baseline & 1 & 0.74 ± 0.04 & 1.43 ± 0.13 & 0.52x \\
macOS desktop baseline & 10 & 7.39 ± 0.37 & 14.26 ± 1.28 & 0.52x \\
macOS desktop baseline & 100 & 71.93 ± 3.55 & 137.37 ± 11.94 & 0.52x \\
macOS desktop baseline & 1000 & 566.75 ± 26.54 & 1003.73 ± 66.39 & 0.56x \\
iOS simulator (debug) & 1 & 0.69 ± 0.06 & 1.34 ± 0.18 & 0.52x \\
iOS simulator (debug) & 10 & 6.89 ± 0.61 & 13.31 ± 1.84 & 0.52x \\
iOS simulator (debug) & 100 & 66.88 ± 5.89 & 128.55 ± 17.19 & 0.52x \\
iOS simulator (debug) & 1000 & 516.79 ± 43.34 & 957.88 ± 100.17 & 0.54x \\
Android emulator (release) & 1 & 0.24 ± 0.04 & 0.93 ± 0.12 & 0.26x \\
Android emulator (release) & 10 & 2.42 ± 0.43 & 9.28 ± 1.21 & 0.26x \\
Android emulator (release) & 100 & 23.92 ± 4.18 & 90.31 ± 11.42 & 0.26x \\
Android emulator (release) & 1000 & 215.45 ± 35.75 & 716.48 ± 69.80 & 0.30x \\
\bottomrule
\end{longtable}
...
\addplot+[
  ...
  (Analyses/s,71.49) +- (0,4.02)
  (Chars/s,71.49) +- (0,4.02)
  (Tokens/s,71.60) +- (0,4.02)
};
\addplot+[
  fill=orange!60,
  ...
  (Analyses/s,100.00) +- (0,2.07)
  (Chars/s,100.00) +- (0,2.07)
  (Tokens/s,100.00) +- (0,2.07)
};
...
\addplot+[
  ...
  (Init(ms),192.15) +- (0,9.36)
  (AvgLatency(ms),140.00) +- (0,6.67)
  (TokenLatency(us/token),139.98) +- (0,8.02)
};
\addplot+[
  fill=orange!60,
  draw=orange!80!black,
  ...
  (Init(ms),100.00) +- (0,10.21)
  (AvgLatency(ms),100.00) +- (0,3.33)
  (TokenLatency(us/token),100.00) +- (0,2.06)
};
\legend{flutter\_kiwi\_nlp,kiwipiepy}
\end{axis}
\end{tikzpicture}
...
\midrule
1 & 1324.26 & 831.03 & 2529.03 & 3375.78 \\
2 & 1428.26 & 670.98 & 2453.40 & 3323.00 \\
3 & 1382.42 & 661.76 & 2227.94 & 3484.31 \\
4 & 1367.82 & 681.92 & 2309.14 & 3362.80 \\
5 & 1254.09 & 670.75 & 2528.08 & 3305.87 \\
\bottomrule
\end{tabular}
\end{table}
...
configurations (roughly 0.4--0.6 ms). This matches the practical impression
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \texttt{numThreads = -1} and \texttt{num\_workers = -1} are both auto modes,
  but they are not guaranteed to map to equivalent parallel execution strategy.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  (desktop \texttt{n=5}, iOS/Android \texttt{n=10}).
...
The current repository test suite contains 49 unit tests organized across
14 groups in 9 test files. Coverage focus is strongest on API contract behavior,
...
Using \texttt{flutter test --coverage test} on February 17, 2026:
\begin{itemize}[leftmargin=1.5em]
  \item Raw line coverage: 324/345 = 93.91\%.
  \item Filtered line coverage: 79/79 = 100.00\%.
\end{itemize}
...
\subsection{Why Raw and Filtered Coverage Differ}
...
must not be interpreted as full end-to-end runtime coverage.

\subsection{Honest Quality Assessment}
The present quality state is strong for deterministic unit-level contract tests
and pure-Dart utility/model layers. Remaining risk concentrates in integration
boundaries:
\begin{itemize}[leftmargin=1.5em]
...
\begin{itemize}[leftmargin=1.5em]
  \item Web runtime depends on module/WASM availability unless self-hosted.
  \item Native archive fallback requires network and writable cache location.
  \item Throughput can trail Python-native execution in some workloads.
  \item Native execution is in-process via FFI; a crash in dependent native
  code is fatal to the host Flutter process.
  \item Platforms outside declared plugin matrix are unsupported.
  \item Current mobile benchmark rows use simulator/emulator environments, not
...
\begin{enumerate}[leftmargin=1.5em]
  \item Persistent model-cache policy controls and eviction settings.
  \item Finer-grained phase timing instrumentation in runtime paths.
  \item Expanded compatibility matrix across Kiwi model families.
  \item Additional deterministic fixtures for difficult tokenization cases.
  \item Optional batch analysis API for high-throughput service workloads.
\end{enumerate}

\section{Conclusion}
\texttt{flutter\_kiwi\_nlp} provides a robust integration architecture for
cross-platform Korean morphological analysis in Flutter. The package deliberately
keeps the public API small while implementing substantial internal mechanics for
runtime parity, build automation, model provisioning, and error transparency.

This revision extends benchmark evidence beyond a desktop-only snapshot by
adding repeated mobile measurements on iOS simulator and Android emulator with
the same corpus and option settings used by the desktop benchmark protocol.

Its practical value is strongest for teams that need one application codebase
across mobile, desktop, and web while retaining predictable analysis semantics
...
arXiv preprint arXiv:1602.05629, 2016. [Online]. Available:
...
February 17, 2026. Build scripts, dependency versions, defaults, and benchmark
numbers may change in future releases.
...
uv run python tool/benchmark/run_compare.py \
  --device "iPhone 17" --mode debug --trials 10 \
  --warmup-runs 5 --measure-runs 30

# Android emulator row (release)
uv run python tool/benchmark/run_compare.py \
  --device <android_emulator_id> --mode release --trials 10 \
  --warmup-runs 5 --measure-runs 30

# iOS physical device row (release; requires attached iPhone)
uv run python tool/benchmark/run_compare.py \
  --device <ios_physical_device_id> --mode release --trials 10 \
  --warmup-runs 5 --measure-runs 30

# Android physical device row (release; requires attached phone)
uv run python tool/benchmark/run_compare.py \
  --device <android_physical_device_id> --mode release --trials 10 \
  --warmup-runs 5 --measure-runs 30
...
\begin{itemize}[leftmargin=1.5em]
  \item \path{benchmark/results/comparison.md}
  \item \path{benchmark/results/ios_debug_t10/comparison.md}
  \item \path{benchmark/results/android_release_t10/comparison.md}
  \item \path{benchmark/results/flutter_kiwi_benchmark_trials.json}
  \item \path{benchmark/results/kiwipiepy_benchmark_trials.json}
