...
\begin{tabularx}{\linewidth}{@{}L{0.42\linewidth}L{0.17\linewidth}L{0.35\linewidth}@{}}
\toprule
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item This revision introduces explicit layered timing decomposition
  (\texttt{pure\_elapsed\_ms}, \texttt{full\_elapsed\_ms},
  \texttt{json\_overhead\_ms}) in addition to warm/cold summaries.
\end{itemize}
...
\begingroup
\footnotesize
\begin{longtable}{@{}L{0.27\linewidth}L{0.17\linewidth}L{0.17\linewidth}L{0.11\linewidth}L{0.16\linewidth}L{0.10\linewidth}@{}}
...
\begin{itemize}[leftmargin=1.5em]
  \item Core package layer (\path{test/}): 51 unit tests in 14 groups across
  9 files.
...
This yields 59 test declarations across repository test directories. Coverage
...
\begin{itemize}[leftmargin=1.5em]
  \item Raw line coverage: 350/374 = 93.58\%.
...
and now includes initial acceptance/golden coverage in the example app.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  self-hosted under application-controlled origins.
  \item Web deployments must satisfy CSP rules for module/WASM fetch origins;
  default CDN bootstrap therefore introduces an explicit supply-chain boundary.
...
\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{Bridge performance redesign:} replace or complement JSON bridge
  payloads with typed/native memory transfer to reduce serialization overhead
  and improve warm-path latency/throughput.
  \item \textbf{Benchmark external validity:} add release/profile measurements
  on physical iOS and Android devices and include sustained Web throughput rows
  under explicit network/cache conditions.
