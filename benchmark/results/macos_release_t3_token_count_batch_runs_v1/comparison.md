...
| generated_at_utc (first trial) | 2026-02-18T02:30:19.633527Z | 2026-02-18T02:30:22Z |
| trials | 3 | 3 |
...
| --- | ---: |
| Pure processing elapsed (ms) | 294.93 ± 18.39 |
| Full analyze elapsed (ms) | 331.75 ± 23.81 |
| JSON overhead elapsed (ms) | 36.81 ± 12.28 |
| JSON overhead per analysis (ms) | 0.0614 ± 0.0205 |
| JSON overhead per token (us) | 3.8109 ± 1.2709 |
| JSON overhead ratio (%) | 11.02 ± 3.34 |
...
| --- | ---: | ---: | ---: |
| Throughput (analyses/s, higher better) | 2039.62 ± 126.71 | 2625.55 ± 61.43 | 0.78x (slower) |
| Throughput (chars/s, higher better) | 69143.10 ± 4295.61 | 89006.28 ± 2082.62 | 0.78x (slower) |
| Throughput (tokens/s, higher better) | 32837.87 ± 2040.10 | 42205.78 ± 987.56 | 0.78x (slower) |
| Avg warm latency (ms, lower better) | 0.49 ± 0.03 | 0.38 ± 0.01 | 1.29x (slower) |
| Avg warm token latency (us/token, lower better) | 30.53 ± 1.90 | 23.70 ± 0.55 | 1.29x (slower) |

## Layered Throughput Breakdown

| Layer | Throughput (mean ± std, analyses/s) |
| --- | ---: |
| Flutter pure (`token_count`) | 2039.62 ± 126.71 |
| Flutter full (`json`) | 1814.59 ± 125.12 |
| kiwipiepy current API path (`analyze`) | 2625.55 ± 61.43 |

| Derived ratio | Value |
| --- | ---: |
| Flutter pure / kiwi | 0.78x (slower) |
| Flutter full / kiwi | 0.69x (slower) |
| Flutter boundary loss (full vs pure) | 11.03% |
...
| --- | ---: | ---: | ---: |
| Init time (ms, lower better) | median 1687.16, p95 1694.71 | median 901.97, p95 939.36 | 1.87x (slower) |
...
| ---: | ---: | ---: | ---: |
| 1 | 0.60 ± 0.01 | 1.11 ± 0.05 | 0.54x (slower) |
| 10 | 5.94 ± 0.09 | 11.05 ± 0.50 | 0.54x (slower) |
| 100 | 57.93 ± 0.92 | 106.46 ± 4.72 | 0.54x (slower) |
| 1000 | 461.23 ± 11.54 | 779.92 ± 30.09 | 0.59x (slower) |

## Per-Trial Raw Snapshot

| Trial | Flutter init (ms) | Kiwi init (ms) | Flutter warm analyses/s | Kiwi warm analyses/s |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 1687.16 | 901.97 | 2041.89 | 2604.60 |
| 2 | 1649.69 | 861.83 | 2165.18 | 2694.72 |
| 3 | 1695.55 | 943.52 | 1911.78 | 2577.34 |
